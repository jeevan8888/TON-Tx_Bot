...
export async function initBot() {
  ...
  telegraf.on("message", async (ctx) => {
    try {
      ...
      console.log("botext------>",ctx.message.text);
      ...
      // @ts-expect-error message.text is string | undefined
      if (config && (ctx.message.text?.startsWith("/track") || ctx.message.text?.startsWith("/stop")) ) {
      // @ts-expect-error message.text is string | undefined
      ...
      if (config.value.tokenRequested && config.tokenAddress === null) {
        ...
        try {
          ...
          if (dbTicker.conmarketcapId === null) {
            const id = await getTONTokenId(tokenAddress);
            dbTicker.conmarketcapId = id;
            console.log("dbTicker_id+++++>",id)
            if (dbTicker.conmarketcapId !== null) {
